...
# yfinance logs a lot of noise by default -- silencing it so only real errors show
...
def get_stock_data(ticker):
    ...
    try:
        ...
        # 3 months feels like the right window -- enough trend data without ballooning the prompt cost
 
        # works for stocks and ETFs now -- ETFs don't have currentPrice so checking all 3
...
"""
    ...
    # print(f"DEBUG: raw ticker response = {raw}")  # left this in during testing
...
def recommendation_mode():
    ...
    queue = list(tickers)
    max_attempts = 15  # cap so we don't loop forever if Claude keeps suggesting bad tickers
...
"""
    ...
    picks = [int(x.strip()) - 1 for x in choice.split(",") if x.strip().isdigit()]
